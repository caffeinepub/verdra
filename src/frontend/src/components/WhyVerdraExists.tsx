import { TreePine, Users, Wind } from "lucide-react";
...
export default function WhyVerdraExists() {
  ...
  const { ref: paraRef, isVisible: paraVisible } = useScrollReveal({
    threshold: 0.2,
  });
  const { ref: purposeTitleRef, isVisible: purposeTitleVisible } =
    useScrollReveal({ threshold: 0.15 });
  const { ref: imageRef, isVisible: imageVisible } = useScrollReveal({
    threshold: 0.1,
  });
  const { ref: purposeTextRef, isVisible: purposeTextVisible } =
    useScrollReveal({ threshold: 0.1 });
  const { ref: moreTitleRef, isVisible: moreTitleVisible } = useScrollReveal({
    threshold: 0.15,
  });
  const { ref: closingRef } = useScrollReveal({
    threshold: 0.3,
  });
  ...
  return (
    <section
      ...
      style={{
        background:
          "linear-gradient(180deg, #050f0a 0%, #071a10 40%, #0a2a1e 70%, #050f0a 100%)",
      }}
    >
      ...
      <div className="relative z-10 pt-28 pb-20 px-4 sm:px-6 max-w-4xl mx-auto text-center">
        ...
        >
          ...
          >
            Urban expansion is accelerating at an unprecedented pace. Streets
            are losing their shade. Heat levels are rising, making cities
            hostile to life. Dust and pollution are increasing with every
            passing season. Citizens feel the urgency — they want to act, to
            contribute, to make a difference — but they lack a structured
            platform to channel that energy into lasting change.
          </p>
        </div>
      </div>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          >
            <div
              ...
              style={{
                boxShadow:
                  "0 0 0 1px rgba(76,175,80,0.25), 0 0 40px rgba(76,175,80,0.12), 0 24px 60px rgba(0,0,0,0.5)",
              }}
            >
              <img
              ...
              <div
                ...
                style={{
                  background:
                    "linear-gradient(to top, rgba(15,61,46,0.6), transparent)",
                }}
              />
              {/* Corner accent */}
              <div
          ...
          >
            <p
              ...
              style={{ color: "rgba(245,241,232,0.65)" }}
            >
              VerdRa was born from a simple but powerful belief — that every
              street deserves shade, every city deserves beauty, and every
              citizen deserves a role in shaping the world they live in.
            </p>
        ...
        >
          ...
          <p
            ...
            style={{
              letterSpacing: "-0.01em",
              textShadow:
                "0 0 40px rgba(76,175,80,0.4), 0 2px 40px rgba(0,0,0,0.5)",
